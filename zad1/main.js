...
function showBeersTable() {
    document.body.innerHTML = '<table id="table"><tr><td>Name</td><td>Type</td><td>Brewery</td> \
                               <td>origin</td><td>bitterness</td><td>alcohol</td><td>hops</td><td>malts</td><tr></table>';
    var i = 0;
    for (i; i < beers.length; i++) {
        document.getElementById('table').innerHTML += '<tr id="td' + i + '"></tr>';
        document.getElementById('td' + i).innerHTML = '<td>' + beers[i].name + '</td><td>' +
            beers[i].type + '</td><td>' + beers[i].brewery +
            '</td><td>' + beers[i].origin + '</td><td>' +
            beers[i].bitterness + '</td><td>' + beers[i].alcohol_content +
            '</td><td>' + beers[i].hops + '</td><td>' + beers[i].malts + '</td>';
    }
}
